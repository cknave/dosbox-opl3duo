...
struct Handler : public Adlib::Handler {
	...
	Handler(const char *device, int baud) :
	...
	void Generate(MixerChannel *chan, Bitu samples) override {
		// Real hardware, not generating anything
		while( samples > 0 ) {
			Bitu todo = samples > 1024 ? 1024 : samples;
			samples -= todo;
			chan->AddSamples_m16( todo, zeros );
		}
	}
	...
	void WriteReg(Bit32u addr, Bit8u val) override {
		...
		const uint8_t buffer[] = {
				uint8_t(((0) << 6) |	        // chip select
				        ((addr & 0x100)) >> 8),	// register high bit
				uint8_t(addr & 0xff),		    // register low bits
				uint8_t(val),			        // value
		};
	...
	struct timespec diffTime(const struct timespec &t1, struct timespec t2)
	{
...
Module::Module( Section* configuration ) : Module_base(configuration) {
	...
	} else if (oplemu == "serial") {
		const char *device = section->Get_string("oplserialport");
		int baud = section->Get_int("oplserialbaud");
		handler = new OPL3DuoSerial::Handler(device, baud);
	} else {
		handler = new DBOPL::Handler();
	}
	handler->Init( rate );
